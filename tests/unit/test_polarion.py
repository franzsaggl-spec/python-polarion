...
from polarion.exceptions import PolarionApiError, PolarionConnectionError
...
def test_get_client_plugins_none_creates_empty_list(mock_polarion):
    ...
    with patch("polarion.polarion.Client") as MockClient, patch("polarion.polarion.requests") as mock_req:
...
def test_download_from_svn_uses_custom_svn_repo_url(mock_polarion):
    ...
    with patch("polarion.polarion.requests") as mock_req:
        ...
        result = mock_polarion.downloadFromSvn("http://polarion.example.com/repo/project/path/file.txt")
...
def test_no_hardcoded_credentials():
    ...
    source_path = os.path.join(os.path.dirname(__file__), "..", "..", "polarion", "polarion.py")
    ...
    for cred in ["aurora"]:
        ...
        assert not re.search(pattern, source, re.IGNORECASE), f'Found hardcoded credential "{cred}" in polarion.py'
