...
class TestPolarionPlan(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.executing_plan = cls.executing_project.createPlan(
            "Test plan" + datetime.now().strftime("%d-%m-%Y-%H-%M-%S-%f"),
    ...
    def test_add_remove_workitem(self):
        ...
        self.assertEqual(
            workitem_package.plannedInURIs.SubterraURI[0],
            self.executing_plan.uri,
            msg="Plan in workitem does not match current plan",
        )
        self.assertEqual(
            len(workitems_in_plan_start) + 1, len(workitems_in_plan_end), msg="Workitems in plan did not increase by 1"
        )
        ...
        self.assertEqual(
            len(workitems_in_plan_start) - 1, len(workitems_in_plan_end), msg="Workitems in plan did not decrease by 1"
        )
    ...
    def test_add_remove_workitem_new_type(self):
        ...
        self.assertEqual(
            self.executing_plan.allowedTypes.EnumOptionId[1].id, workitem_task.type.id, msg="workitem type not added"
        )

        self.executing_plan.addToPlan(workitem_task)

        self.assertEqual(
            workitem_task.plannedInURIs.SubterraURI[0],
            self.executing_plan.uri,
            msg="Plan in workitem does not match current plan",
        )
    ...
    def test_parent_children(self):
        ...
        self.assertEqual(len(self.checking_plan.getChildren()), 0, msg="No children should be here")

        # create a child and check that there are children.
        child_a = self.executing_project.createPlan(
            "Test plan" + datetime.now().strftime("%d-%m-%Y-%H-%M-%S-%f"),
            datetime.now().strftime("%d-%m-%Y-%H-%M-%S-%f"),
            "iteration",
            new_plan_parent=self.executing_plan.id,
        )

        self.assertEqual(
            len(self.checking_plan.getChildren()), 1, msg="Added one child, but this is not in checking plan"
        )
        ...
        sub_child = self.executing_project.createPlan(
            "Test plan" + datetime.now().strftime("%d-%m-%Y-%H-%M-%S-%f"),
            datetime.now().strftime("%d-%m-%Y-%H-%M-%S-%f"),
            "iteration",
