...
class TestPolarionTestRecord(unittest.TestCase):
    ...
    def test_attachment_test_step(self):
        for step_index in range(1):
            ...
            self.assertFalse(
                self.executing_test_record.testStepHasAttachment(step_index), msg="Test step already has attachments"
            )
            ...
            self.assertTrue(
                self.executing_test_record.testStepHasAttachment(step_index), msg="Test step does not have attachments"
            )
            self.assertFalse(
                self.executing_test_record.testStepHasAttachment(step_index + 1),
                msg="Test step already has attachments",
            )

            attachment_file1 = (
                self.executing_test_record.testStepResults.TestStepResult[step_index]
                .attachments.TestRunAttachment[0]
                .fileName
            )
            ...
            attachment_file2 = (
                self.executing_test_record.testStepResults.TestStepResult[step_index]
                .attachments.TestRunAttachment[1]
                .fileName
            )
            ...
            self.assertFalse(
                self.executing_test_record.testStepHasAttachment(step_index), msg="Test step still has attachments"
            )
