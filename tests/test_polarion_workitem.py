...
class TestPolarionWorkitem(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.checking_project = cls.pol.getProject(polarion_project_id)
    ...
    def test_create_workitem(self):
        ...
        checking_workitem = self.checking_project.getWorkitem(executed_workitem.id)

        self.assertEqual(executed_workitem, checking_workitem, msg="Workitems not identical")
        ...
        self.assertEqual(
            executed_workitem, checking_workitem, msg="Workitems not identical when created with supplied fields"
        )

        # Check that fields are checks for validity
        with self.assertRaises(Exception):
            executed_workitem = self.executing_project.createWorkitem(
                "task", new_workitem_fields={"wrong-field": "what happens?"}
            )
    ...
    def test_description_change(self):
        ...
        checking_workitem = self.checking_project.getWorkitem(executed_workitem.id)
        self.assertNotEqual(
            new_description, checking_workitem.getDescription(), msg="Description cannot be the same before setting"
        )
        ...
        checking_workitem = self.checking_project.getWorkitem(executed_workitem.id)
        self.assertEqual(
            new_description, checking_workitem.getDescription(), msg="description is not the same after setting"
        )
        self.assertEqual(
            executed_workitem.getDescription(),
            checking_workitem.getDescription(),
    ...
    def test_add_comment_not_available(self):
        ...
        polarion = Polarion(polarion_url, polarion_user, polarion_password)

        executing_project = polarion.getProject(polarion_project_id)
    ...
    def test_assignee(self):
        self.assertEqual(
            len(self.global_workitem.getAssignedUsers()), 0, msg="Testing workitem should have no assigned users"
        )
        ...
        checking_workitem = self.checking_project.getWorkitem(self.global_workitem.id)

        executing_users = self.global_workitem.getAssignedUsers()
        checking_users = checking_workitem.getAssignedUsers()

        self.assertEqual(len(executing_users), 1, msg="executing workitem should have 1 assigned users")
        self.assertEqual(len(checking_users), 1, msg="checking workitem should have 1 assigned users")
        ...
        self.assertEqual(len(executing_users), 0, msg="executing workitem should have 0 assigned users")
        self.assertEqual(len(checking_users), 0, msg="checking workitem should have 0 assigned users")

    def test_replace_assignee(self):
        self.assertGreater(
            len(self.executing_project.getUsers()), 1, msg="Project does not have more than 1 user in project"
        )
        ...
        self.assertEqual(len(executing_users), 1, msg="executing workitem should have 0 assigned users")
        self.assertEqual(len(checking_users), 1, msg="checking workitem should have 0 assigned users")
    ...
    def test_get_enum(self, mock_project):
        ...
        self.assertEqual(expected_return, actual_return, msg="Return value not equal to mock")
        ...
        actual_return = self.global_workitem.getStatusEnum()
        mock_project.assert_called_with("task-status")
        self.assertEqual(expected_return, actual_return, msg="Return value not empty array")
    ...
    def test_workitem_compare(self):
        ...
        self.assertNotEqual(executed_workitem, checking_workitem, msg="Workitems identical, which is wrong")
    ...
    def test_workitem_creator(self):
        ...
        self.assertEqual(self.global_workitem, new_workitem, msg="Workitems not identical")
    ...
    def test_workitem_author_creator(self):
        ...
        self.assertEqual(self.global_workitem.author.id, author.id, msg="Authors not identical")
    ...
    def test_set_status(self):
        ...
        self.assertNotEqual(
            executed_workitem.status.id, new_value, msg="Workitem should not have updated to illegal value"
        )
    ...
    def test_add_link(self):
        ...
        self.assertEqual(
            checking_workitem_1.uri, checking_workitem_2.linkedWorkItemsDerived.LinkedWorkItem[0].workItemURI
        )
    ...
    def test_approvee(self):
        ...
        self.assertEqual(
            executed_workitem_1.approvals.Approval[0].user.id,
            testing_user_1.id,
            msg="Approving user not equal to assign user",
        )
        self.assertEqual(
            checking_workitem_1.approvals.Approval[0].user.id,
            testing_user_1.id,
            msg="Approving user not equal to assign user",
        )
    ...
    def test_resolution(self):
        ...
        executed_workitem_1.getResolutionEnum()
    ...
    def test_testcase_add_remove_steps(self):
        ...
        self.assertEqual(
            0, len(executed_workitem_1.getTestSteps()), msg="Length of test steps is not 0 when creating a new item"
        )
        ...
        # check length again
        self.assertEqual(3, len(executed_workitem_1.getTestSteps()), msg="Length of test steps is not 3")

        # remove middle test step
        executed_workitem_1.removeTestStep(1)

        # check length to be 1 less
        self.assertEqual(2, len(executed_workitem_1.getTestSteps()), msg="Length of test steps is not 2")
    ...
    def test_testcase_update_steps(self):
        ...
        for i in range(3):
            self.assertEqual(
                f"{i}",
                executed_workitem_1.getTestSteps()[i]["step"],
                msg="Value in first column changes since creation",
            )
            self.assertEqual(
                f"Test step {i}",
                executed_workitem_1.getTestSteps()[i]["description"],
                msg="Value in second column changes since creation",
            )
            self.assertEqual(
                f"nothing here {i}",
                executed_workitem_1.getTestSteps()[i]["expectedResult"],
                msg="Value in third column changes since creation",
            )
        ...
        for i in range(3):
            self.assertEqual(
                f"{10 + i}", executed_workitem_1.getTestSteps()[i]["step"], msg="Value in first column did not change"
            )
            self.assertEqual(
                f"new {i + 20}",
                executed_workitem_1.getTestSteps()[i]["description"],
                msg="Value in second column did not change",
            )
            self.assertEqual(
                f"last {i + 20}",
                executed_workitem_1.getTestSteps()[i]["expectedResult"],
                msg="Value in third column did not change",
            )
    ...
    def test_get_linked_items(self):
        ...
        self.assertEqual(executed_workitem_2, executed_workitem_1.getLinkedItemWithRoles()[0][1], msg="Check workitem")
        self.assertEqual("follow_up", executed_workitem_1.getLinkedItemWithRoles()[0][0], msg="Check link type")
        self.assertEqual(executed_workitem_2, executed_workitem_1.getLinkedItem()[0], msg="Check workitem")

        self.assertEqual(executed_workitem_1, executed_workitem_2.getLinkedItemWithRoles()[0][1], msg="Check workitem")
        self.assertEqual("follow_up", executed_workitem_2.getLinkedItemWithRoles()[0][0], msg="Check link type")
        self.assertEqual(executed_workitem_1, executed_workitem_2.getLinkedItem()[0], msg="Check workitem")
