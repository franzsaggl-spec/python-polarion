...
class TestPolarionTestrun(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.executing_test_run = cls.executing_project.createTestRun(
    ...
    def test_record_list(self):
        ...
        for r in checking_testrun.records:
            ...
            self.assertTrue(
                self.executing_test_run.hasTestCase(r.getTestCaseName()),
                msg="Test run does not have test case it should have",
            )
            ...
            self.assertEqual(
                executing_record.getTestCaseName(), checking_record.getTestCaseName(), msg="Records did not match"
            )

        self.assertFalse(
            self.executing_test_run.hasTestCase(list_record[0] + "invalid"), msg="Invalid test case was available"
        )
        self.assertIsNone(
            self.executing_test_run.getTestCase(list_record[0] + "invalid"), msg="Invalid test case was returned"
        )
    ...
    def test_add_testcase(self):
        ...
        self.assertEqual(
            num_of_test_records_before + 1,
            len(self.executing_test_run.records),
            msg="Length of records did not increase",
        )
        self.assertEqual(
            num_of_test_records_before + 1, len(checking_testrun.records), msg="Length of records did not increase"
        )
