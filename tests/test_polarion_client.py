...
class TestPolarionClient(unittest.TestCase):
    def test_wrong_url(self):
        ...
        self.assertRaises(Exception, Polarion.__init__, url, polarion_user, polarion_password)

    def test_wrong_user(self):
        user = polarion_user + "incorrect"
        self.assertRaises(Exception, Polarion.__init__, polarion_url, user, polarion_password)

    def test_wrong_password(self):
        password = polarion_password + "incorrect"
        self.assertRaises(Exception, Polarion.__init__, polarion_url, polarion_user, password)

    def test_available_services_static(self):
        known_services = ["Session", "Project", "Tracker", "Builder", "Planning", "TestManagement", "Security"]

        pol = Polarion(polarion_url, polarion_user, polarion_password, static_service_list=True)

        for service in known_services:
            self.assertTrue(pol.hasService(service), msg="Service should exist")

        self.assertFalse(pol.hasService("made_up"), msg="Service should not exist")
    ...
    def test_types(self):
        ...
        self.assertIn("ArrayOfTestStepResult", str(type(pol.ArrayOfTestStepResultType)))
        self.assertIn("TestStepResult", str(type(pol.TestStepResultType)))

    def test_type_wrong_service(self):
        pol = Polarion(polarion_url, polarion_user, polarion_password)

        self.assertRaises(Exception, pol.getTypeFromService, "made_up", "dont care")
    ...
    def test_project_creation(self, mock_project):
        ...
        pol.getProject("Random_id")
        mock_project.assert_called_with(pol, "Random_id")

        pol.getProject("other_project")
    ...
    def test_alternative_repo(self):
        ...
        pol = Polarion(
            polarion_url, polarion_user, polarion_password, svn_repo_url=polarion_url.replace("polarion", "repoext")
        )
        project = pol.getProject(polarion_project_id)
        testrun = project.createTestRun(
