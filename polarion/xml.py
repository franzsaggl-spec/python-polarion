...
class Config:
    ...
    def _check_mandatory(self) -> None:
        ...
        if getattr(self, Config.TOKEN) is None and (getattr(self, Config.USERNAME) is None or getattr(self, Config.PASSWORD) is None):
        ...
        if test_case.tag == XmlParser.TEST_CASE:
            ...
            for elem in test_case:
                if elem.tag in ['error', 'failure','skipped']:
                    ...
                    for prop in elem:
                        if XmlParser.PROPERTY == prop.tag and 'name' in prop.attrib.keys() and 'value' in prop.attrib.keys():
            ...
            if custom_field is not None:
                ...
                if comment is not None:
                    ...
                    if len(split) == 2:
                        ...
                        logger.warning(f'unable to parse properly {Importer.TEST_RUN_COMMENT_CUSTOM_FIELD} of testrun: {comment}. So it is not updated')
